...
const SYSTEM_PROMPT = [
  ...
  '  "markers": [ { "x": 0-100 的横向百分比, "y": 0-100 的纵向百分比, "type": "good | warn", "label": "部位名，4 字以内，如 门襟纽扣 / 左袖口" } ],',
  ...
  '· 每个 marker 都要给 label —— 图上直接写这个部位名，读者不用去对编号。',
  '  名字要短（4 字以内）且和 findings 里说的是同一个部位，别一个写「门襟」一个写「前襟」。',
  '  label 实在想不出就不写，界面会退回数字编号。'
].join('\n');
